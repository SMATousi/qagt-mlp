...
import os as _os
# figures are written to <repo>/figures unless QAGT_FIG_DIR overrides it
_FIG = _os.environ.get('QAGT_FIG_DIR') or _os.path.join(
    _os.path.dirname(_os.path.abspath(__file__)).split('docs/demos')[0], 'figures')
_os.makedirs(_FIG, exist_ok=True)
...
plt.tight_layout()
plt.savefig(_os.path.join(_FIG, 'heldout-step-extrapolation.png'),dpi=200)
print('saved')
